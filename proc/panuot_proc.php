<?php 
    ...
    if($_REQUEST["accion"]=="buscar_orden"){
        ...
        $fec = '';
        $est = '';
        $query ="SELECT OTPQRREPO,OTTECN,OTFEAS,OTESTA FROM ot WHERE OTDEPA = $dep AND OTLOCA = $loc AND OTNUME = $ord AND ( OTESTA=0 OR OTESTA=1 )";
        $respuesta = $conn->prepare($query) or die ($sql);
        if(!$respuesta->execute()) return false;
        if($respuesta->rowCount()>0){
            while ($row=$respuesta->fetch()){
    ...
    if($_REQUEST["accion"]=="anular_orden"){
    	$dep = $_REQUEST["dep"];
    	$loc = $_REQUEST["loc"];
    	$ord = $_REQUEST["ord"];

    	$query ="UPDATE ot SET OTTECN = 0, OTESTA = 4 WHERE OTDEPA=$dep AND OTLOCA=$loc AND OTNUME=$ord";
