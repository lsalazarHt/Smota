<?php 
    ...
    if($_REQUEST["accion"]=="buscar_orden"){
        ...
        $est = '';
        //AND OTESTA=1
        $query ="SELECT OTPQRREPO,OTTECN,OTFEAS,OTESTA FROM ot WHERE OTDEPA = $dep AND OTLOCA = $loc AND OTNUME = $ord ";
        $respuesta = $conn->prepare($query) or die ($sql);
        if(!$respuesta->execute()) return false;
        if($respuesta->rowCount()>0){
            while ($row=$respuesta->fetch()){
    ...
    if($_REQUEST["accion"]=="redimir_orden"){
    	$dep = $_REQUEST["dep"];
    	$loc = $_REQUEST["loc"];
    	$ord = $_REQUEST["ord"];
    	$obs = $_REQUEST["obs"];

    	/*$query ="UPDATE ot SET OTESTA = 2, OTOBSERVAS= '$obs' WHERE OTDEPA=$dep AND OTLOCA=$loc AND OTNUME=$ord";
