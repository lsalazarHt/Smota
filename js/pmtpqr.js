$(document).ready(function () {
	...
	$('#btnNuevo').click(function(){
		...
		if( (cod!='') && (nom!='') ){
		    ...
		    cell3.className = '';
		    cell3.innerHTML = '<input type="text" id="txtCantMax'+rowCount+'" class="form-control input-sm text-center" onclick="swEditor(\'txtNomb'+rowCount+'\',\'trSelect'+rowCount+'\',0,0)">';
		    //Accion
		    var cell4 = row.insertCell(3);
		    cell4.className = 'text-center ';
		    cell4.innerHTML = '<input type="checkbox" id="txtCkek'+rowCount+'" onclick="swEditor(\'\',\'trSelect'+rowCount+'\',0,0)" checked="checked">';
	...
	$('#btnGuardar').click(function(){
		...
		for(var i=1;i<=cont;i++){
			...
			if( $('#txtTipo'+i).val() == 1){ //Editar
				...
				$.ajax({
			        type:'POST',
			        url:'proc/pmtpqr_proc.php?accion=editar_registros',
			        data:{ codOrg:codOrg, codPqr:codPqr, cod:cod, cant:cant, chek:chek},
				...
				if( (cod!='') && (nom!='') ){
					$.ajax({
				        type:'POST',
				        url:'proc/pmtpqr_proc.php?accion=guardar_registros',
				        data:{ codPqr:codPqr, cod:cod, cant:cant , chek:chek},
				        success: function(data){
				            if(data==1){
			                    actualizar();
...
function cargarPqr(){
	...
	$.ajax({
        type:'POST',
        url:'proc/pmtpqr_proc.php?accion=cargar_pqrs',
        data:{codDep:'1'},
        success: function(data){
...
function buscarNombrePqr(){
	...
	if(cod!=''){
		$.ajax({
	        type:'POST',
	        url:'proc/pmtpqr_proc.php?accion=seleccionar_pqr',
	        data:{cod:cod},
	        success: function(data){
	            $('#txtNomPqr').val(data);
...
function colocarMaterial(cod){
	...
	if(!sw){
		$.ajax({
	        type:'POST',
	        url:'proc/pmtpqr_proc.php?accion=seleccionar_material',
	        data:{cod:cod},
	        success: function(data){
...
function actualizar(){
	...
	$.ajax({
        type:'POST',
        url:'proc/pmtpqr_proc.php?accion=actualizar_registros',
        data:{cod:cod},
        success: function(data){
            $('#table').html(data);
...
function limpiarTabla(){
	...
	d = '<th class="text-center" width="230">CANTIDAD MAXIMA A LEGALIZAR</th>';
	e = '<th class="text-center" width="100">VALOR FIJO</th>';
	f = '</tr></thead><tbody></tbody>';
	$('#table').html(a+b+c+d+e+f);
}
