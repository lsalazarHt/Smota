$(document).ready(function () {
	...
	$('#btnGuardar').click(function(){
		...
		for(var i=1;i<=cont;i++){
			...
			if( $('#txtTipo'+i).val() == 1){ //Editar
				$.ajax({
			        type:'POST',
			        url:'proc/psect_proc.php?accion=editar_registros',
			        data:{codOrg:codOrg, codDep:codDep, codLoc:codLoc, cod:cod , nom:nom, chek:chek},
				...
				if( (cod!='') && (nom!='') ){
					$.ajax({
				        type:'POST',
				        url:'proc/psect_proc.php?accion=guardar_registros',
				        data:{codDep:codDep, codLoc:codLoc, cod:cod, nom:nom},
				        success: function(data){
				            if(data==1){
			                    actualizar();
	...
	$('#btnConsulta').click(function(){
		...
		$.ajax({
	        type:'POST',
	        url:'proc/psect_proc.php?accion=cargar_departamentos',
	        data:{dep:'1'},
	        success: function(data){
	        	swCons = true;
...
function buscarLocalidades(cod){
	$.ajax({
        type:'POST',
        url:'proc/psect_proc.php?accion=seleccionar_departamento',
        data:{cod:cod},
        success: function(data){
            $('#txtCodDepar').val(cod);
...
function actualizarCiudades(dep){
	...
	$.ajax({
        type:'POST',
        url:'proc/psect_proc.php?accion=cargar_ciudades',
        data:{dep:dep},
        success: function(data){
...
function buscarNombreLocalidades(cod){
	...
	if(cod!=''){
		$.ajax({
	        type:'POST',
	        url:'proc/psect_proc.php?accion=seleccionar_localidad',
...
function actModalLocalidades(){
	...
	if(dep!=''){
		...
		$.ajax({
	        type:'POST',
	        url:'proc/psect_proc.php?accion=act_localidades',
	        data:{dep:dep},
	        success: function(data){
...
function actualizar(){
	...
	$.ajax({
        type:'POST',
        url:'proc/psect_proc.php?accion=actualizar_sectores',
